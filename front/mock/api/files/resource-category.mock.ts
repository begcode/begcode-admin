...
const fetchData = () => {
  if (!feteched) {
    allData.push(...getMockData('resource_category'));
    feteched = true;
  }
};
...
export function setResourceCategoryMock(mock) {
  ...
  mock.onGet(`${baseApiUrl}/tree`).reply(config => {
    fetchData();
    const { params = '' } = config;
    const { page = '0', size = '15' } = qs.parse(params);
    return [200, resultPageSuccess(Number(page), Number(size), allData)];
  });
  mock.onGet(`${baseApiUrl}/\\d+/tree`).reply(config => {
    fetchData();
    const { params = '' } = config;
    const { page = '0', size = '15' } = qs.parse(params);
    return [200, resultPageSuccess(Number(page), Number(size), allData)];
