...
const getPopConfirmAttrs = computed(() => {
  return attrs => {
    const originAttrs = _omit(attrs, ['confirm', 'cancel', 'icon']);
    if (!attrs.onConfirm && attrs.confirm && _isFunction(attrs.confirm)) originAttrs['onConfirm'] = attrs.confirm;
    if (!attrs.onCancel && attrs.cancel && _isFunction(attrs.cancel)) originAttrs['onCancel'] = attrs.cancel;
    return originAttrs;
  };
});
