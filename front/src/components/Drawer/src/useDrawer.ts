...
export function useDrawer(): UseDrawerReturnType {
  ...
  const getInstance = () => {
    ...
    if (!instance) {
      const log = useLog();
      log.prettyError('useDrawer instance is undefined!');
    }
    return instance;
  };
  ...
  const methods: ReturnMethods = {
    ...
    openDrawer: <T = any>(open = true, data?: T, openOnSet = true): void => {
      ...
      const equal = _isEqual(toRaw(dataTransferRef[unref(uid)]), toRaw(data));
      if (!equal) {
        dataTransferRef[unref(uid)] = toRaw(data);
      }
    },
...
export const useDrawerInner = (callbackFn?: Fn): UseDrawerInnerReturnType => {
  ...
  const getInstance = () => {
    ...
    if (!instance) {
      const log = useLog();
      log.prettyError('useDrawerInner instance is undefined!');
      return;
    }
    return instance;
  };
  ...
  watchEffect(() => {
    ...
    if (!data) return;
    if (!callbackFn || !_isFunction(callbackFn)) return;
    nextTick(() => {
      callbackFn(data);
    });
  });
