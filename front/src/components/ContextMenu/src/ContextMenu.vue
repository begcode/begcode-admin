  ...
  setup(props) {
    ...
    function renderMenuItem(items: ContextMenuItem[]) {
      ...
      return visibleItems.map(item => {
        ...
        if (!children || children.length === 0) {
          return (
            <>
              <a-menu-item disabled={disabled} class={`${prefixCls}__item`} key={label}>
                <ItemContent {...contentProps} />
              </a-menu-item>
              {divider ? <a-divider key={`d-${label}`} /> : null}
            </>
          );
        }
        ...
        return (
          <a-menu-sub-menu key={label} disabled={disabled} popupClassName={`${prefixCls}__popup`}>
            {{
    ...
    return () => {
      ...
      return (
        <div class={prefixCls}>
          return (
          <a-menu inlineIndent={12} mode="vertical" class={prefixCls} ref={wrapRef} style={unref(getStyle)}>
