...
export default defineComponent({
  ...
  setup(props, { attrs, slots, emit, expose }) {
    ...
    const getBindValues = computed(() => {
      ...
      return _omit(propsData, 'treeData', 'class') as TreeProps;
    });

    const handleCheck = (v: CheckKeys, e?) => {
      let currentValue = toRaw(state.checkedKeys) as KeyType[];
      if (_isArray(currentValue) && searchState.startSearch && e) {
        const value = e.node.eventKey;
        currentValue = _difference(currentValue, getChildrenKeys(value));
        if (e.checked) {
    ...
    function getIcon(params: TreeItem, icon?: string) {
      if (!icon) {
        if (props.renderIcon && _isFunction(props.renderIcon)) {
    ...
    async function handleRightClick({ event, node }: Recordable) {
      ...
      if (beforeRightClick && _isFunction(beforeRightClick)) {
    ...
    function renderAction(node: TreeItem) {
      ...
      return actionList.map((item, index) => {
        let nodeShow = true;
        if (_isFunction(item.show)) {
          nodeShow = item.show?.(node);
        } else if (_isBoolean(item.show)) {
          nodeShow = item.show;
        }
    ...
    const treeData = computed(() => {
      const data = _cloneDeep(getTreeSearchData.value);
      eachTree(data, (item, _parent) => {
        ...
        const title = _get(item, titleField);

        const searchIdx = searchText ? title.indexOf(searchText) : -1;
        const isHighlight = searchState.startSearch && !isEmpty(searchText) && highlight && searchIdx !== -1;
        const highlightStyle = `color: ${_isBoolean(highlight) ? '#f50' : highlight}`;
    ...
    return () => {
      ...
      return (
        <div class={[bem(), 'h-full', attrs.class]}>
          ...
          <a-spin wrapperClassName={unref(props.treeWrapperClassName)} spinning={unref(props.loading)} tip="加载中...">
            <ScrollContainer style={scrollStyle} v-show={!unref(getNotFound)}>
              <a-tree ref={treeRef} {...unref(getBindValues)} showIcon={false} treeData={treeData.value}>
                {extendSlots(slots, ['title'])}
              </a-tree>
            </ScrollContainer>
            <a-empty v-show={unref(getNotFound)} image={Empty.PRESENTED_IMAGE_SIMPLE} class="!mt-4" />
          </a-spin>
        </div>
      );
    };
