...
export default defineComponent({
  ...
  setup(props, { slots }) {
    ...
    const getDisable = computed(() => {
      ...
      if (_isBoolean(dynamicDisabled)) {
        disabled = dynamicDisabled;
      }
      if (_isFunction(dynamicDisabled)) {
    ...
    const getReadonly = computed(() => {
      ...
      if (_isBoolean(dynamicReadonly)) {
        readonly = dynamicReadonly;
      }
      if (_isFunction(dynamicReadonly)) {
    ...
    function getShow(): { isShow: boolean; isIfShow: boolean } {
      ...
      const itemIsAdvanced = showAdvancedButton ? (_isBoolean(props.isAdvanced) ? props.isAdvanced : true) : true;
      ...
      if (_isBoolean(ifShow)) {
        isIfShow = ifShow;
      }
      if (_isFunction(show)) {
        isShow = show(unref(getValues));
      }
      if (_isFunction(ifShow)) {
    ...
    function handleRules(): ValidationRule[] {
      ...
      if (_isFunction(dynamicRules)) {
        return dynamicRules(unref(getValues)) as ValidationRule[];
      }

      let rules: ValidationRule[] = _cloneDeep(defRules) as ValidationRule[];
      ...
      function validator(rule: any, value: any) {
        ...
        if (value === undefined || _isNull(value)) {
          // 空值
      ...
      const getRequired = _isFunction(required) ? required(unref(getValues)) : required;
    ...
    function renderComponent() {
      ...
      const eventKey = `on${_upperFirst(changeEvent)}`;
      ...
      const compSlot = _isFunction(renderComponentContent)
        ? {
    ...
    function renderLabelHelpMessage() {
      ...
      const getHelpMessage = _isFunction(helpMessage) ? helpMessage(unref(getValues)) : helpMessage;
    ...
    function renderItem() {
      ...
      if (component === 'Divider') {
        ...
        return (
          <a-form-item labelCol={labelCol} wrapperCol={wrapperCol} name={field} class={{ 'suffix-item': !!suffix }}>
        ...
        const getSuffix = _isFunction(suffix) ? suffix(unref(getValues)) : suffix;
