...
export default {
  ...
  create(smsTemplate: ISmsTemplate): Promise<ISmsTemplate> {
    return defHttp.post({ url: `${apiUrl}`, data: smsTemplate });
  },
  ...
  update(smsTemplate: ISmsTemplate, batchIds?: number[], batchFields?: String[]): Promise<ISmsTemplate> {
    ...
    return defHttp.put({ url: `${apiUrl}/${smsTemplate.id}?${queryParams}`, data: smsTemplate });
  },
  ...
  syncFromSupplier(supplierId: number): Promise<any> {
    return defHttp.get({ url: `${apiUrl}/sync-from-supplier/${supplierId}` });
  },
