...
const apiUrl = '/api/form-save-data';

export default {
  // begcode-please-regenerate-this-file 如果您不希望重新生成代码时被覆盖，将please修改为don't ！！！

  find(id: number): Promise<IFormSaveData> {
    return defHttp.get({ url: `${apiUrl}/${id}` });
  },

  retrieve(paginationQuery?: any): Promise<PageRecord<IFormSaveData>> {
  ...
  create(formSaveData: IFormSaveData): Promise<IFormSaveData> {
    return defHttp.post({ url: `${apiUrl}`, data: formSaveData });
  },

  update(formSaveData: IFormSaveData, batchIds?: number[], batchFields?: String[]): Promise<IFormSaveData> {
    let queryParams = '';
    if (batchIds && batchFields) {
      queryParams = qs.stringify({ batchIds, batchFields }, { arrayFormat: 'repeat' });
    }
    return defHttp.put({ url: `${apiUrl}/${formSaveData.id}?${queryParams}`, data: formSaveData });
  },
