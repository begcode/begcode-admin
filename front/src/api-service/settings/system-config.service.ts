...
export default {
  ...
  find(id: number): Promise<ISystemConfig> {
    return defHttp.get({ url: `${apiUrl}/${id}` });
  },

  retrieve(paginationQuery?: any): Promise<PageRecord<ISystemConfig>> {
  ...
  create(systemConfig: ISystemConfig): Promise<ISystemConfig> {
    return defHttp.post({ url: `${apiUrl}`, data: systemConfig });
  },

  update(systemConfig: ISystemConfig, batchIds?: number[], batchFields?: String[]): Promise<ISystemConfig> {
    let queryParams = '';
    if (batchIds && batchFields) {
      queryParams = qs.stringify({ batchIds, batchFields }, { arrayFormat: 'repeat' });
    }
    return defHttp.put({ url: `${apiUrl}/${systemConfig.id}?${queryParams}`, data: systemConfig });
  },
