...
import ViewPermissionEdit from './components/form-component.vue';
  ...
  switch (code) {
    ...
    case 'new':
      const containerConfig: any = {
        componentName: shallowRef(ViewPermissionEdit),
        entityId: '',
        title: '新建',
        okText: '保存',
        cancelText: '取消',
  ...
  if (operation?.click) {
    ...
    switch (name) {
      case 'save':
        break;
      case 'edit':
        containerConfig.componentName = shallowRef(ViewPermissionEdit);
        containerConfig.title = '编辑可视权限';
      ...
      case 'detail':
        if (operation?.containerType) {
          containerConfig.componentName = shallowRef(ViewPermissionDetail);
