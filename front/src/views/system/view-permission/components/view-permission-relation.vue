<template>
  <div data-cy="ViewPermissionRelationHeading">
    <a-card
...
import ViewPermissionDetail from './detail-component.vue';
import ViewPermissionEdit from './form-component.vue';
...
const relationships = {
  'Authority.viewPermissions': 'authorities',
};
...
const config = {
  ...
  baseGridOptions: (): VxeGridProps => {
    return {
      ...
      showOverflow: true,
      keepSource: true,
      id: 'vxe_grid_viewPermission_relation',
      printConfig: {
        columns: [
          // { field: 'name' },
        ],
...
const apis = {
  ...
  findByParentId: apiService.system.viewPermissionService.treeByParentId,
...
const toolbarClick = ({ code }) => {
  ...
  switch (code) {
    ...
    case 'add': {
      ...
      popupConfig.componentProps.is = shallowRef(ViewPermissionList);
    ...
    case 'new':
      ...
      popupConfig.containerProps.showOkBtn = true;
      popupConfig.containerProps.showCancelBtn = true;
      popupConfig.componentProps.is = shallowRef(ViewPermissionEdit);
      popupConfig.componentProps.entityId = '';
      popupConfig.componentProps.baseData = props.baseData;
...
const rowClick = ({ name, data }) => {
  ...
  if (operation?.click) {
    ...
    switch (name) {
      case 'detail':
        popupConfig.componentProps.is = shallowRef(ViewPermissionDetail);
