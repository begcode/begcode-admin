<template>
  ...
  <div data-cy="ApiPermissionHeading">
    <a-card
      ...
      <vxe-grid ref="xGrid" v-bind="gridOptions" v-on="gridEvents" data-cy="entityTable">
        ...
        <template #authorities_default="{ row, column, $grid }">
          <AvatarGroupInfo
            ...
            :query="{ apiPermissionsId: row.id, 'id.aggregate.count': true }"
            :stats-api="async params => (await apis.authoritiesStats(params).catch(() => [{ id_count: null }]))[0]"
...
import ApiPermissionDetail from './components/detail-component.vue';
...
const shallowRefs = {
  ApiPermissionEdit: shallowRef(ApiPermissionForm),
  ApiPermissionDetail: shallowRef(ApiPermissionDetail),
...
const apis = {
  ...
  authoritiesStats: apiService.system.authorityService.stats,
...
useSetShortcutButtons('SystemApiPermissionList', extraButtons);
...
function toolbarClick({ code }) {
  ...
  switch (code) {
      ...
      if (records?.length > 0) {
        ...
        Modal.confirm({
          ...
          okButtonProps: {
            ...
            'data-cy-heading': 'apiPermissionDeleteDialogHeading',
          } as any,
        });
      }
      break;
    ...
    case 'new':
      ...
      popupConfig.componentProps.is = shallowRefs.ApiPermissionEdit;
...
function rowClick({ name, data, params }) {
  ...
  if (operation?.click) {
    ...
    switch (name) {
      ...
      case 'edit':
        popupConfig.needSubmit = true;
        popupConfig.containerProps.title = '编辑API权限';
      ...
      case 'detail':
        ...
        popupConfig.containerProps.showCancelBtn = true;
        popupConfig.componentProps.is = shallowRefs.ApiPermissionDetail;
        popupConfig.componentProps.entityId = row.id;
