  ...
  switch (code) {
    ...
    case 'new':
      const containerConfig: any = {
        componentName: shallowRef(ApiPermissionEdit),
        entityId: '',
        title: '新建',
        okText: '保存',
        cancelText: '取消',
  ...
  if (operation?.click) {
    ...
    switch (name) {
      case 'save':
        break;
      case 'edit':
        containerConfig.componentName = shallowRef(ApiPermissionEdit);
        containerConfig.title = '编辑API权限';
      ...
      case 'detail':
        if (operation?.containerType) {
          containerConfig.componentName = shallowRef(ApiPermissionDetail);
