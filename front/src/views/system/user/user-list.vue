...
import UserEdit from './components/form-component.vue';
  ...
  switch (code) {
    ...
    case 'new':
      const containerConfig: any = {
        componentName: shallowRef(UserEdit),
        entityId: '',
        title: '新建',
        okText: '保存',
        cancelText: '取消',
  ...
  if (operation?.click) {
    ...
    switch (name) {
      case 'save':
        break;
      case 'edit':
        containerConfig.componentName = shallowRef(UserEdit);
        containerConfig.title = '编辑用户';
      ...
      case 'detail':
        if (operation?.containerType) {
          containerConfig.componentName = shallowRef(UserDetail);
