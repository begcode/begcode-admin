<template>
  <div :class="containerType === 'page' ? ['pb-44px'] : []" data-cy="UserCreateUpdateHeading">
    <basic-form ref="formRef" v-bind="formProps">
      <template #resetBefore>
        <teleport to='[data-teleport="system/user-edit-append-button"]' defer>
          <a-space>
...
const getEntityData = async (entityId: string | number) => {
  if (entityId) {
    ...
    const data = await apiService.system.userService.find(Number(entityId)).catch(() => null);
    if (data) {
      Object.assign(user, data);
    }
  } else {
    Object.assign(user, props.baseData);
  }
};
...
const saveOrUpdateApi = props.entityId ? apiService.system.userService.update : apiService.system.userService.create;
const submit = async (config = { submitToServer: true }) => {
  ...
  if (result.success) {
    ...
    const saveData = await saveOrUpdateApi(user).catch(() => false);
    if (saveData) {
