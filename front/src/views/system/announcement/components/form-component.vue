<template>
  <div :class="containerType === 'page' ? ['pb-44px'] : []" data-cy="AnnouncementCreateUpdateHeading">
    <basic-form ref="formRef" v-bind="formProps">
      <template #resetBefore>
        <teleport to='[data-teleport="system/announcement-edit-append-button"]' defer>
          <a-space>
...
const announcementId = ref<any>(null);
const announcement = reactive<IAnnouncement>(new Announcement());
const getEntityData = async (entityId: string | number) => {
  if (entityId) {
    announcementId.value = entityId;
    const data = await apiService.system.announcementService.find(Number(entityId)).catch(() => null);
    if (data) {
      Object.assign(announcement, data);
    }
  } else {
    Object.assign(announcement, props.baseData);
  }
};
...
const saveOrUpdateApi = props.entityId ? apiService.system.announcementService.update : apiService.system.announcementService.create;
const submit = async (config = { submitToServer: true }) => {
  ...
  if (result.success) {
    Object.assign(announcement, result.data);
    if (!config.submitToServer) {
      return announcement;
    }
    const saveData = await saveOrUpdateApi(announcement).catch(() => false);
    if (saveData) {
      Object.assign(announcement, saveData);
