  ...
  switch (code) {
    ...
    case 'new':
      const containerConfig: any = {
        componentName: shallowRef(AnnouncementEdit),
        entityId: '',
        title: '新建',
        okText: '保存',
        cancelText: '取消',
  ...
  if (operation?.click) {
    ...
    switch (name) {
      case 'save':
        break;
      case 'edit':
        containerConfig.componentName = shallowRef(AnnouncementEdit);
        containerConfig.title = '编辑系统通告';
      ...
      case 'detail':
        if (operation?.containerType) {
          containerConfig.componentName = shallowRef(AnnouncementDetail);
