<template>
  <PageWrapper v-bind="pageProps">
    <template #default>
      <AnnouncementForm ref="announcementFormRef" v-bind="formProps" @updateSaveButton="updateSaveButton"></AnnouncementForm>
    </template>
...
import AnnouncementForm from './components/form-component.vue';
...
let announcementId = ref(route.params?.entityId as string);

const saveOrUpdate = async () => {
  const result = await announcementFormRef.value.submit();
  if (result) {
    if (!announcementId.value) {
      const { fullPath } = route;
      tabStore.closeTabByKey(fullPath, router).then(() => {
        go('/system/announcement/' + result.id + '/edit', true);
      });
    }
  }
};
...
const operationsConfig = ref<any>([
  ...
  {
    title: announcementId.value ? '更新' : '保存',
    icon: '',
    type: 'primary',
    name: 'save',
    click: saveOrUpdate,
