<template>
  <div :class="containerType === 'page' ? ['pb-44px'] : []" data-cy="SmsMessageCreateUpdateHeading">
    <basic-form ref="formRef" v-bind="formProps">
      <template #resetBefore>
        <teleport to='[data-teleport="system/sms-message-edit-append-button"]' defer>
          <a-space>
...
const smsMessageId = ref<any>(null);
const smsMessage = reactive<ISmsMessage>(new SmsMessage());
const getEntityData = async (entityId: string | number) => {
  if (entityId) {
    smsMessageId.value = entityId;
    const data = await apiService.system.smsMessageService.find(Number(entityId)).catch(() => null);
    if (data) {
