  ...
  <div data-cy="AuthorityHeading">
    ...
    <a-card :bordered="false" class="bc-list-result-card" :bodyStyle="{ 'padding-top': '1px' }">
      <vxe-grid ref="xGrid" v-bind="gridOptions" v-on="gridEvents" data-cy="entityTable">
        ...
        <template #viewPermissions_default="{ row, column, $grid }">
          <AvatarGroupInfo
            :value="row.viewPermissions"
            :disabled="!$grid?.isEditByRow(row)"
            :query="{ 'id.in': row.viewPermissions }"
            avatar-slot-name="default"
            avatar-slot-field="text"
            avatar-tip-field="text"
            @click="buttonType => rowClick({ name: column.field + 'Column' + _upperFirst(buttonType), data: row, params: column.params })"
          />
        </template>
        <template #apiPermissions_default="{ row, column, $grid }">
          <AvatarGroupInfo
            :value="row.apiPermissions"
            :disabled="!$grid?.isEditByRow(row)"
            :query="{ 'id.in': row.apiPermissions }"
            avatar-slot-name="default"
        ...
        <template #users_default="{ row, column, $grid }">
          <AvatarGroupInfo
            :disabled="!$grid?.isEditByRow(row)"
            :query="{ authoritiesId: row.id, 'id.aggregate.count': true }"
            :stats-api="async params => (await apis.usersStats(params).catch(() => [{ id_count: null }]))[0]"
            avatar-slot-name="default"
            avatar-slot-field="firstName"
            avatar-tip-field="id_count"
            @click="buttonType => rowClick({ name: column.field + 'Column' + _upperFirst(buttonType), data: row, params: column.params })"
          />
        </template>
        <template #pagerLeft>
...
import AuthorityDetail from './components/detail-component.vue';
import config from './config/list-config';
import { ApiTree } from '@/components/Form';
import { upperFirst as _upperFirst } from 'lodash-es';
import UserRelation from '@/views/system/user/components/user-relation.vue';
...
const shallowRefs = {
  AuthorityEdit: shallowRef(AuthorityForm),
  AuthorityDetail: shallowRef(AuthorityDetail),
  ApiTree: shallowRef(ApiTree),
  UserRelation: shallowRef(UserRelation),
...
const apis = {
  ...
  import: apiService.system.authorityService.importExcel,
  export: apiService.system.authorityService.exportExcel,
  viewPermissionsList: apiService.system.viewPermissionService.tree,
  apiPermissionsList: apiService.system.apiPermissionService.tree,
...
useSetShortcutButtons('SystemAuthorityList', extraButtons);
...
function toolbarClick({ code }) {
  ...
  switch (code) {
      ...
      if (records?.length > 0) {
        ...
        Modal.confirm({
          ...
          okButtonProps: {
            ...
            'data-cy-heading': 'authorityDeleteDialogHeading',
          } as any,
        });
      }
      break;
    ...
    case 'new':
      ...
      popupConfig.componentProps.is = shallowRefs.AuthorityEdit;
...
function rowClick({ name, data, params }) {
  ...
  if (operation?.click) {
    ...
    switch (name) {
      ...
      case 'edit':
        popupConfig.needSubmit = true;
        popupConfig.containerProps.title = '编辑角色';
      ...
      case 'detail':
        ...
        popupConfig.containerProps.showCancelBtn = true;
        popupConfig.componentProps.is = shallowRefs.AuthorityDetail;
        popupConfig.componentProps.entityId = row.id;
      ...
      case 'delete':
        ...
        break;
      case 'viewPermissionsColumnView': {
        popupConfig.containerProps.title = '关联可视权限';
        ...
        const rowParams = {
          ...
          api: apis.viewPermissionsList,
          resultField: 'records',
          disabled: true,
          multiple: true,
          checkedKeys: (row.viewPermissions || []).map(item => item.id),
        };
      ...
      case 'viewPermissionsColumnEdit': {
        popupConfig.containerProps.title = '关联可视权限';
        popupConfig.containerProps.okText = '保存';
        popupConfig.containerProps.cancelText = '关闭';
        popupConfig.needSubmit = false;
        ...
        popupConfig.componentEvents = {
          check: ({ checked }: any) => {
            ...
            row.viewPermissions = checked.map(id => ({ id: id }));
          },
        };
        popupConfig.containerEvents = {
          ok: async () => {
            const viewPermissions = popupConfig.componentProps.checkedKeys.map(id => ({ id: id }));
            const data = await apis.update({ ...row, viewPermissions }, [row.id], ['viewPermissions']).catch(err => {
              console.log(err);
              message.error('保存失败！');
            });
            if (data) {
              xGrid.value.reloadRow(row, data, 'viewPermissions');
      ...
      case 'apiPermissionsColumnView': {
        popupConfig.containerProps.title = '关联API权限';
        popupConfig.containerProps.okText = '';
        popupConfig.containerProps.cancelText = '关闭';
        popupConfig.needSubmit = false;
        popupConfig.containerProps.showOkBtn = false;
        ...
        const rowParams = {
          ...
          api: apis.apiPermissionsList,
          resultField: 'records',
          disabled: true,
          multiple: true,
          checkedKeys: (row.apiPermissions || []).map(item => item.id),
        };
        Object.assign(popupConfig.componentProps, rowParams);
        if (rowParams.containerType === 'drawer') {
          popupConfig.containerProps.showFooter = true;
      ...
      case 'apiPermissionsColumnEdit': {
        ...
        popupConfig.componentEvents = {
          check: ({ checked }: any) => {
            popupConfig.componentProps.checkedKeys = checked;
            row.apiPermissions = checked.map(id => ({ id: id }));
          },
        };
        popupConfig.containerEvents = {
          ok: async () => {
            const apiPermissions = popupConfig.componentProps.checkedKeys.map(id => ({ id: id }));
            const data = await apis.update({ ...row, apiPermissions }, [row.id], ['apiPermissions']).catch(err => {
              console.log(err);
              message.error('保存失败！');
            });
            if (data) {
              xGrid.value.reloadRow(row, data, 'apiPermissions');
              message.success('保存成功！');
              closeDrawer();
            }
          },
      ...
      case 'usersColumnView': {
        ...
        popupConfig.containerProps.showOkBtn = false;
        popupConfig.containerProps.showCancelBtn = true;
        popupConfig.componentProps.is = shallowRefs.UserRelation;
        const rowParams = {
          ...params,
          containerType: 'modal',
          cardExtra: [],
