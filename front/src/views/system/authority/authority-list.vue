      ...
      case 'viewPermissionsColumnEdit': {
        ...
        popupConfig.containerEvents = {
          ok: async () => {
            ...
            const data = await apis.update({ id: row.id, viewPermissions }, [row.id], ['viewPermissions']).catch(err => {
              console.log(err);
              message.error('保存失败！');
            });
            if (data) {
      ...
      case 'apiPermissionsColumnEdit': {
        ...
        popupConfig.containerEvents = {
          ok: async () => {
            ...
            const data = await apis.update({ id: row.id, apiPermissions }, [row.id], ['apiPermissions']).catch(err => {
              console.log(err);
              message.error('保存失败！');
            });
            if (data) {
