  ...
  switch (code) {
    ...
    case 'new':
      const containerConfig: any = {
        componentName: shallowRef(AuthorityEdit),
        entityId: '',
        title: '新建',
        okText: '保存',
        cancelText: '取消',
  ...
  if (operation?.click) {
    ...
    switch (name) {
      case 'save':
        break;
      case 'edit':
        containerConfig.componentName = shallowRef(AuthorityEdit);
        containerConfig.title = '编辑角色';
      ...
      case 'detail':
        if (operation?.containerType) {
          containerConfig.componentName = shallowRef(AuthorityDetail);
