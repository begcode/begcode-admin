<template>
  <div :class="containerType === 'page' ? ['pb-44px'] : []" data-cy="AuthorityCreateUpdateHeading">
...
const getEntityData = async (entityId: string | number) => {
  if (entityId) {
    authorityId.value = entityId;
    const data = await apiService.system.authorityService.find(Number(entityId)).catch(() => null);
    if (data) {
      Object.assign(authority, data);
    }
  } else {
    Object.assign(authority, props.baseData);
  }
};
...
const saveOrUpdateApi = props.entityId ? apiService.system.authorityService.update : apiService.system.authorityService.create;
const submit = async (config = { submitToServer: true }) => {
  ...
  if (result.success) {
    Object.assign(authority, result.data);
    if (!config.submitToServer) {
      return authority;
    }
    const saveData = await saveOrUpdateApi(authority).catch(() => false);
    if (saveData) {
      Object.assign(authority, saveData);
