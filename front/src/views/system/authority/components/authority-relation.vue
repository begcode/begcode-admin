<template>
  <div data-cy="AuthorityRelationHeading">
    <a-card
...
import AuthorityDetail from './detail-component.vue';
import AuthorityEdit from './form-component.vue';
...
const config = {
  ...
  baseGridOptions: (): VxeGridProps => {
    return {
      ...
      showOverflow: true,
      keepSource: true,
      id: 'vxe_grid_authority_relation',
      printConfig: {
        columns: [
          // { field: 'name' },
        ],
...
const apis = {
  ...
  findByParentId: apiService.system.authorityService.treeByParentId,
...
const toolbarClick = ({ code }) => {
  ...
  switch (code) {
    ...
    case 'add': {
      ...
      popupConfig.componentProps.is = shallowRef(AuthorityList);
    ...
    case 'new':
      ...
      popupConfig.containerProps.showOkBtn = true;
      popupConfig.containerProps.showCancelBtn = true;
      popupConfig.componentProps.is = shallowRef(AuthorityEdit);
      popupConfig.componentProps.entityId = '';
      popupConfig.componentProps.baseData = props.baseData;
...
const rowClick = ({ name, data }) => {
  ...
  if (operation?.click) {
    ...
    switch (name) {
      case 'detail':
        popupConfig.componentProps.is = shallowRef(AuthorityDetail);
