  ...
  switch (code) {
    ...
    case 'new':
      const containerConfig: any = {
        componentName: shallowRef(TaskJobConfigEdit),
        entityId: '',
        title: '新建',
        okText: '保存',
        cancelText: '取消',
  ...
  if (operation?.click) {
    ...
    switch (name) {
      case 'save':
        break;
      case 'edit':
        containerConfig.componentName = shallowRef(TaskJobConfigEdit);
        containerConfig.title = '编辑定时任务';
      ...
      case 'detail':
        if (operation?.containerType) {
          containerConfig.componentName = shallowRef(TaskJobConfigDetail);
