<template>
  ...
  <div data-cy="TaskJobConfigHeading">
    <a-card
...
import TaskJobConfigDetail from './components/detail-component.vue';
...
const shallowRefs = {
  TaskJobConfigEdit: shallowRef(TaskJobConfigForm),
  TaskJobConfigDetail: shallowRef(TaskJobConfigDetail),
};
...
const apis = {
  ...
  import: apiService.taskjob.taskJobConfigService.importExcel,
  export: apiService.taskjob.taskJobConfigService.exportExcel,
};
const pageConfig = {
  title: '任务配置列表',
...
useSetShortcutButtons('TaskjobTaskJobConfigList', extraButtons);
...
function toolbarClick({ code }) {
  ...
  switch (code) {
      ...
      if (records?.length > 0) {
        ...
        Modal.confirm({
          ...
          okButtonProps: {
            ...
            'data-cy-heading': 'taskJobConfigDeleteDialogHeading',
          } as any,
        });
      }
      break;
    ...
    case 'new':
      ...
      popupConfig.componentProps.is = shallowRefs.TaskJobConfigEdit;
...
function rowClick({ name, data, params }) {
  ...
  if (operation?.click) {
    ...
    switch (name) {
      ...
      case 'edit':
        popupConfig.needSubmit = true;
        popupConfig.containerProps.title = '编辑任务配置';
      ...
      case 'detail':
        ...
        popupConfig.containerProps.showCancelBtn = true;
        popupConfig.componentProps.is = shallowRefs.TaskJobConfigDetail;
        popupConfig.componentProps.entityId = row.id;
