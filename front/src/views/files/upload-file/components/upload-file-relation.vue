<template>
  <div data-cy="UploadFileRelationHeading">
...
import UploadFileDetail from './detail-component.vue';
import UploadFileEdit from './form-component.vue';
...
const config = {
  ...
  baseGridOptions: (): VxeGridProps => {
    return {
      ...
      showOverflow: true,
      keepSource: true,
      id: 'vxe_grid_uploadFile_relation',
      printConfig: {
        columns: [
          // { field: 'name' },
        ],
...
const apis = {
  uploadFileService: apiService.files.uploadFileService,
  ...
  copy: apiService.files.uploadFileService.copy,
  import: apiService.files.uploadFileService.importExcel,
  export: apiService.files.uploadFileService.exportExcel,
...
const toolbarClick = ({ code }) => {
  ...
  switch (code) {
    ...
    case 'add': {
      ...
      popupConfig.componentProps.is = shallowRef(UploadFileList);
    ...
    case 'new':
      ...
      popupConfig.containerProps.showOkBtn = true;
      popupConfig.containerProps.showCancelBtn = true;
      popupConfig.componentProps.is = shallowRef(UploadFileEdit);
      popupConfig.componentProps.entityId = '';
      popupConfig.componentProps.baseData = props.baseData;
      popupConfig.containerEvents.ok = async () => {
        const result = await drawerComponentRef.value.submit({ submitToServer: true });
        if (result) {
          // 对新增的记录进行关联关系处理
          if (props.editIn === 'drawer') {
            closeDrawer();
...
const okModal = async () => {
  if (modalComponentRef.value) {
    ...
    if (props.updateType === 'remoteApi') {
      // 对selectRows进行处理。
      const newRows = selectRows.map(row => Object.assign(row, props.baseData));
...
const rowClick = ({ name, data }) => {
  ...
  if (operation?.click) {
    ...
    switch (name) {
      case 'detail':
        popupConfig.componentProps.is = shallowRef(UploadFileDetail);
