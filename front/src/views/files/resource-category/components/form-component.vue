<template>
  <div :class="containerType === 'page' ? ['pb-44px'] : []" data-cy="ResourceCategoryCreateUpdateHeading">
    <basic-form ref="formRef" v-bind="formProps">
      <template #resetBefore>
        <teleport to='[data-teleport="files/resource-category-edit-append-button"]' defer>
          <a-space>
...
const resourceCategoryId = ref<any>(null);
const resourceCategory = reactive<IResourceCategory>(new ResourceCategory());
const getEntityData = async (entityId: string | number) => {
  if (entityId) {
    resourceCategoryId.value = entityId;
    const data = await apiService.files.resourceCategoryService.find(Number(entityId)).catch(() => null);
    if (data) {
      Object.assign(resourceCategory, data);
    }
  } else {
    Object.assign(resourceCategory, props.baseData);
  }
};
...
const saveOrUpdateApi = props.entityId ? apiService.files.resourceCategoryService.update : apiService.files.resourceCategoryService.create;
const submit = async (config = { submitToServer: true }) => {
  ...
  if (result.success) {
    Object.assign(resourceCategory, result.data);
    if (!config.submitToServer) {
      return resourceCategory;
    }
    const saveData = await saveOrUpdateApi(resourceCategory).catch(() => false);
    if (saveData) {
      Object.assign(resourceCategory, saveData);
