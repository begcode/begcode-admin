  ...
  switch (code) {
    ...
    case 'new':
      const containerConfig: any = {
        componentName: shallowRef(SmsSupplierEdit),
        entityId: '',
        title: '新建',
        okText: '保存',
        cancelText: '取消',
  ...
  if (operation?.click) {
    ...
    switch (name) {
      case 'save':
        break;
      case 'edit':
        containerConfig.componentName = shallowRef(SmsSupplierEdit);
        containerConfig.title = '编辑短信服务商配置';
      ...
      case 'detail':
        if (operation?.containerType) {
          containerConfig.componentName = shallowRef(SmsSupplierDetail);
