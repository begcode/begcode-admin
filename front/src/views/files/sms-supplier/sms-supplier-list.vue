<template>
  ...
  <div data-cy="SmsSupplierHeading">
    <a-card
...
import SmsSupplierDetail from './components/detail-component.vue';
...
const shallowRefs = {
  SmsSupplierEdit: shallowRef(SmsSupplierForm),
  SmsSupplierDetail: shallowRef(SmsSupplierDetail),
};
...
const apis = {
  ...
  import: apiService.files.smsSupplierService.importExcel,
  export: apiService.files.smsSupplierService.exportExcel,
};
const pageConfig = {
  title: '短信服务商配置列表',
...
useSetShortcutButtons('SystemSmsSupplierList', extraButtons);
...
function toolbarClick({ code }) {
  ...
  switch (code) {
      ...
      if (records?.length > 0) {
        ...
        Modal.confirm({
          ...
          okButtonProps: {
            ...
            'data-cy-heading': 'smsSupplierDeleteDialogHeading',
          } as any,
        });
      }
      break;
    ...
    case 'new':
      ...
      popupConfig.componentProps.is = shallowRefs.SmsSupplierEdit;
...
function rowClick({ name, data, params }) {
  ...
  if (operation?.click) {
    ...
    switch (name) {
      ...
      case 'edit':
        popupConfig.needSubmit = true;
        popupConfig.containerProps.title = '编辑短信服务商配置';
      ...
      case 'detail':
        ...
        popupConfig.containerProps.showCancelBtn = true;
        popupConfig.componentProps.is = shallowRefs.SmsSupplierDetail;
        popupConfig.componentProps.entityId = row.id;
