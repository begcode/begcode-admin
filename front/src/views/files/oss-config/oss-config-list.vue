  ...
  switch (code) {
    ...
    case 'new':
      const containerConfig: any = {
        componentName: shallowRef(OssConfigEdit),
        entityId: '',
        title: '新建',
        okText: '保存',
        cancelText: '取消',
  ...
  if (operation?.click) {
    ...
    switch (name) {
      case 'save':
        break;
      case 'edit':
        containerConfig.componentName = shallowRef(OssConfigEdit);
        containerConfig.title = '编辑对象存储配置';
      ...
      case 'detail':
        if (operation?.containerType) {
          containerConfig.componentName = shallowRef(OssConfigDetail);
