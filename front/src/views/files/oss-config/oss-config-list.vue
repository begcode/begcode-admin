...
import OssConfigDetail from './components/detail-component.vue';
...
const shallowRefs = {
  OssConfigEdit: shallowRef(OssConfigForm),
  OssConfigDetail: shallowRef(OssConfigDetail),
};
...
const apis = {
  ...
  import: apiService.files.ossConfigService.importExcel,
  export: apiService.files.ossConfigService.exportExcel,
};
const pageConfig = {
  title: '对象存储配置列表',
...
useSetShortcutButtons('OssOssConfigList', extraButtons);
...
function toolbarClick({ code }) {
  ...
  switch (code) {
      ...
      if (records?.length > 0) {
        ...
        Modal.confirm({
          ...
          okButtonProps: {
            ...
            'data-cy-heading': 'ossConfigDeleteDialogHeading',
          } as any,
        });
      }
      break;
    ...
    case 'new':
      ...
      popupConfig.componentProps.is = shallowRefs.OssConfigEdit;
...
function rowClick({ name, data, params }) {
  ...
  if (operation?.click) {
    ...
    switch (name) {
      ...
      case 'edit':
        popupConfig.needSubmit = true;
        popupConfig.containerProps.title = '编辑对象存储配置';
      ...
      case 'detail':
        ...
        popupConfig.containerProps.showCancelBtn = true;
        popupConfig.componentProps.is = shallowRefs.OssConfigDetail;
        popupConfig.componentProps.entityId = row.id;
