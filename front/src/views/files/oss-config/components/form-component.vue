...
const ossConfigId = ref<any>(null);
const ossConfig = reactive<IOssConfig>(new OssConfig());
const getEntityData = async (entityId: string | number) => {
  if (entityId) {
    ossConfigId.value = entityId;
    const data = await apiService.files.ossConfigService.find(Number(entityId)).catch(() => null);
    if (data) {
      Object.assign(ossConfig, data);
    }
  } else {
    Object.assign(ossConfig, props.baseData);
  }
};
...
const saveOrUpdateApi = props.entityId ? apiService.files.ossConfigService.update : apiService.files.ossConfigService.create;
const submit = async (config = { submitToServer: true }) => {
  ...
  if (result.success) {
    Object.assign(ossConfig, result.data);
    if (!config.submitToServer) {
      return ossConfig;
    }
    const saveData = await saveOrUpdateApi(ossConfig).catch(() => false);
    if (saveData) {
      Object.assign(ossConfig, saveData);
