...
import FormConfigDetail from './components/detail-component.vue';
import config from './config/list-config';
import FormConfigDesign from './components/design-component.vue';
import FormConfigCreate from './components/create-component.vue';
...
const shallowRefs = {
  FormConfigEdit: shallowRef(FormConfigForm),
  FormConfigDetail: shallowRef(FormConfigDetail),
};
...
const apis = {
  ...
  import: apiService.settings.formConfigService.importExcel,
  export: apiService.settings.formConfigService.exportExcel,
...
const rowOperations = ref<any[]>([
  ...
  },
  {
    title: '数据',
    name: 'edit-data',
    hide: row => xGrid.value.isEditByRow(row) && xGrid.value.props.editConfig?.mode === 'row' && row.formType !== 'DATA_FORM',
...
useSetShortcutButtons('SystemFormConfigList', extraButtons);
...
function toolbarClick({ code }) {
  ...
  switch (code) {
      ...
      if (records?.length > 0) {
        ...
        Modal.confirm({
          ...
          okButtonProps: {
            ...
            'data-cy-heading': 'formConfigDeleteDialogHeading',
          } as any,
        });
      }
      break;
    ...
    case 'new':
      ...
      popupConfig.componentProps.is = shallowRefs.FormConfigEdit;
...
function rowClick({ name, data, params }) {
  ...
  if (operation?.click) {
    ...
    switch (name) {
      ...
      case 'edit':
        popupConfig.needSubmit = true;
        popupConfig.containerProps.title = '编辑表单配置';
      ...
      case 'detail':
        ...
        popupConfig.containerProps.showCancelBtn = true;
        popupConfig.componentProps.is = shallowRefs.FormConfigDetail;
        popupConfig.componentProps.entityId = row.id;
      ...
      case 'delete':
        ...
        break;
      case 'design':
        popupConfig.containerProps.closable = false;
        popupConfig.containerProps.defaultFullscreen = true;
        popupConfig.containerProps.title = '';
        popupConfig.containerProps.canFullscreen = false;
        popupConfig.containerProps.footer = null;
        popupConfig.componentProps.is = shallowRef(FormConfigDesign);
        popupConfig.componentProps.entityId = row.id;
        popupConfig.componentProps.containerType = 'modal';
        setModalProps({ open: true });
        break;
      case 'edit-data':
        popupConfig.needSubmit = true;
        popupConfig.containerProps = {
          title: '编辑-' + row.formName,
          cancelText: '关闭',
          okText: '保存',
          showOkBtn: true,
          showCancelBtn: true,
        ...
        popupConfig.componentProps = {
          is: shallowRef(FormConfigCreate),
          entityId: row.id,
          containerType: 'modal',
          viewType: 'edit',
        };
