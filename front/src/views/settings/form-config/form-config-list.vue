...
import FormConfigDesign from './form-config-design.vue';
...
const toolbarClick = ({ code }) => {
  ...
  switch (code) {
    ...
    case 'new':
      ...
      break;
    case 'import': {
      apis.formConfigService.generate().then(_res => {
  ...
  if (operation?.click) {
    ...
    switch (name) {
      ...
      case 'delete':
        ...
        break;
      case 'design':
        modalConfig.componentName = shallowRef(FormConfigDesign);
        modalConfig.entityId = row.id;
        modalConfig.containerType = 'modal';
        modalConfig.footer = null;
        modalConfig.canFullscreen = false;
        modalConfig.closable = false;
        setModalProps({ open: true, defaultFullscreen: true, title: '' });
        break;
      default:
