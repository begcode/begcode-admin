<template>
  <div :class="containerType === 'page' ? ['pb-44px'] : []" data-cy="FormConfigCreateUpdateHeading">
    <basic-form ref="formRef" v-bind="formProps">
      <template #resetBefore>
        <teleport to='[data-teleport="settings/form-config-edit-append-button"]' defer>
          <a-space>
...
const formConfigId = ref<any>(null);
const formConfig = reactive<IFormConfig>(new FormConfig());
const getEntityData = async (entityId: string | number) => {
  if (entityId) {
    formConfigId.value = entityId;
    const data = await apiService.settings.formConfigService.find(Number(entityId)).catch(() => null);
    if (data) {
...
const submit = async (config = { submitToServer: true }) => {
  ...
  if (result.success) {
    ...
    const saveData = await saveOrUpdateApi(formConfig).catch(() => false);
    if (saveData) {
