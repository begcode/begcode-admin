  ...
  switch (code) {
    ...
    case 'new':
      const containerConfig: any = {
        componentName: shallowRef(BusinessTypeEdit),
        entityId: '',
        title: '新建',
        okText: '保存',
        cancelText: '取消',
  ...
  if (operation?.click) {
    ...
    switch (name) {
      case 'save':
        break;
      case 'edit':
        containerConfig.componentName = shallowRef(BusinessTypeEdit);
        containerConfig.title = '编辑业务类型';
      ...
      case 'detail':
        if (operation?.containerType) {
          containerConfig.componentName = shallowRef(BusinessTypeDetail);
