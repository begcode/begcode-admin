<template>
  <div :class="containerType === 'page' ? ['pb-44px'] : []" data-cy="BusinessTypeCreateUpdateHeading">
    <basic-form ref="formRef" v-bind="formProps">
      <template #resetBefore>
        <teleport to='[data-teleport="settings/business-type-edit-append-button"]' defer>
          <a-space>
...
const businessTypeId = ref<any>(null);
const businessType = reactive<IBusinessType>(new BusinessType());
const getEntityData = async (entityId: string | number) => {
  if (entityId) {
    businessTypeId.value = entityId;
    const data = await apiService.settings.businessTypeService.find(Number(entityId)).catch(() => null);
    if (data) {
