<template>
  <div data-cy="DictionaryRelationHeading">
    <a-card
...
import DictionaryDetail from './detail-component.vue';
import DictionaryEdit from './form-component.vue';
...
const config = {
  ...
  baseGridOptions: (): VxeGridProps => {
    return {
      ...
      showOverflow: true,
      keepSource: true,
      id: 'vxe_grid_dictionary_relation',
      printConfig: {
        columns: [
          // { field: 'name' },
        ],
...
const toolbarClick = ({ code }) => {
  ...
  switch (code) {
    ...
    case 'add': {
      ...
      popupConfig.componentProps.is = shallowRef(DictionaryList);
    ...
    case 'new':
      ...
      popupConfig.containerProps.showOkBtn = true;
      popupConfig.containerProps.showCancelBtn = true;
      popupConfig.componentProps.is = shallowRef(DictionaryEdit);
      popupConfig.componentProps.entityId = '';
      popupConfig.componentProps.baseData = props.baseData;
...
const rowClick = ({ name, data }) => {
  ...
  if (operation?.click) {
    ...
    switch (name) {
      case 'detail':
        popupConfig.componentProps.is = shallowRef(DictionaryDetail);
