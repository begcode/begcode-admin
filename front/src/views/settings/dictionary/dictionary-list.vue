  ...
  switch (code) {
    ...
    case 'new':
      const containerConfig: any = {
        componentName: shallowRef(DictionaryEdit),
        entityId: '',
        title: '新建',
        okText: '保存',
        cancelText: '取消',
  ...
  if (operation?.click) {
    ...
    switch (name) {
      case 'save':
        break;
      case 'edit':
        containerConfig.componentName = shallowRef(DictionaryEdit);
        containerConfig.title = '编辑数据字典';
      ...
      case 'detail':
        if (operation?.containerType) {
          containerConfig.componentName = shallowRef(DictionaryDetail);
