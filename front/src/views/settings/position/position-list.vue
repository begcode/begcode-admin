  ...
  switch (code) {
    ...
    case 'new':
      const containerConfig: any = {
        componentName: shallowRef(PositionEdit),
        entityId: '',
        title: '新建',
        okText: '保存',
        cancelText: '取消',
  ...
  if (operation?.click) {
    ...
    switch (name) {
      case 'save':
        break;
      case 'edit':
        containerConfig.componentName = shallowRef(PositionEdit);
        containerConfig.title = '编辑岗位';
      ...
      case 'detail':
        if (operation?.containerType) {
          containerConfig.componentName = shallowRef(PositionDetail);
