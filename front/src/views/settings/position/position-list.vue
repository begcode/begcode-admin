      ...
      <vxe-grid ref="xGrid" v-bind="gridOptions" v-on="gridEvents" data-cy="entityTable">
        ...
        <template #users_default="{ row, column }">
          <AvatarGroupInfo
            :value="row.users"
            :disabled="true"
            :query="{ positionId: row.id, 'id.aggregate.count': true }"
...
import PositionDetail from './components/detail-component.vue';
...
const shallowRefs = {
  PositionEdit: shallowRef(PositionForm),
  PositionDetail: shallowRef(PositionDetail),
...
const apis = {
  ...
  import: apiService.settings.positionService.importExcel,
  export: apiService.settings.positionService.exportExcel,
...
useSetShortcutButtons('SystemPositionList', extraButtons);
...
function toolbarClick({ code }) {
  ...
  switch (code) {
    ...
    case 'new':
      ...
      popupConfig.componentProps.is = shallowRefs.PositionEdit;
...
function rowClick({ name, data, params }) {
  ...
  if (operation?.click) {
    ...
    switch (name) {
      ...
      case 'edit':
        popupConfig.needSubmit = true;
        popupConfig.containerProps.title = '编辑岗位';
      ...
      case 'detail':
        ...
        popupConfig.containerProps.showCancelBtn = true;
        popupConfig.componentProps.is = shallowRefs.PositionDetail;
        popupConfig.componentProps.entityId = row.id;
