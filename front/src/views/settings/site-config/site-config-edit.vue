...
const siteConfigFormRef = ref<any>(null);
const pageProps = reactive({
  title: '编辑',
});
let siteConfigId = ref(route.params?.entityId as string);

const saveOrUpdate = async () => {
  const result = await siteConfigFormRef.value.submit();
  if (result) {
    if (!siteConfigId.value) {
      const { fullPath } = route;
      tabStore.closeTabByKey(fullPath, router).then(() => {
        go('/settings/site-config/' + result.id + '/edit', true);
      });
    }
  }
};
