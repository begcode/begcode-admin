  ...
  switch (code) {
    ...
    case 'new':
      const containerConfig: any = {
        componentName: shallowRef(SiteConfigEdit),
        entityId: '',
        title: '新建',
        okText: '保存',
        cancelText: '取消',
  ...
  if (operation?.click) {
    ...
    switch (name) {
      case 'save':
        break;
      case 'edit':
        containerConfig.componentName = shallowRef(SiteConfigEdit);
        containerConfig.title = '编辑网站配置';
      ...
      case 'detail':
        if (operation?.containerType) {
          containerConfig.componentName = shallowRef(SiteConfigDetail);
