<template>
  ...
  <div data-cy="SystemConfigHeading">
    <a-card
...
import SystemConfigDetail from './components/detail-component.vue';
...
const shallowRefs = {
  SystemConfigEdit: shallowRef(SystemConfigForm),
  SystemConfigDetail: shallowRef(SystemConfigDetail),
...
const apis = {
  ...
  update: apiService.settings.systemConfigService.update,
...
const pageConfig = {
  title: '网站配置列表',
  baseRouteName: 'systemSystemConfig',
};
...
useSetShortcutButtons('SystemSystemConfigList', extraButtons);
...
function toolbarClick({ code }) {
  ...
  switch (code) {
      ...
      if (records?.length > 0) {
        ...
        Modal.confirm({
          ...
          okButtonProps: {
            ...
            'data-cy-heading': 'systemConfigDeleteDialogHeading',
          } as any,
        });
      }
      break;
    ...
    case 'new':
      ...
      popupConfig.componentProps.is = shallowRefs.SystemConfigEdit;
...
function rowClick({ name, data, params }) {
  ...
  if (operation?.click) {
    ...
    switch (name) {
      ...
      case 'edit':
        popupConfig.needSubmit = true;
        popupConfig.containerProps.title = '编辑网站配置';
      ...
      case 'detail':
        ...
        popupConfig.containerProps.showCancelBtn = true;
        popupConfig.componentProps.is = shallowRefs.SystemConfigDetail;
        popupConfig.componentProps.entityId = row.id;
