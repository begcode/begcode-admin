  ...
  switch (code) {
    ...
    case 'new':
      const containerConfig: any = {
        componentName: shallowRef(CommonFieldDataEdit),
        entityId: '',
        title: '新建',
        okText: '保存',
        cancelText: '取消',
  ...
  if (operation?.click) {
    ...
    switch (name) {
      case 'save':
        break;
      case 'edit':
        containerConfig.componentName = shallowRef(CommonFieldDataEdit);
        containerConfig.title = '编辑通用字段数据';
      ...
      case 'detail':
        if (operation?.containerType) {
          containerConfig.componentName = shallowRef(CommonFieldDataDetail);
