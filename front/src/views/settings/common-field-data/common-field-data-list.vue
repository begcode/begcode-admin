...
import CommonFieldDataEdit from './components/form-component.vue';
import CommonFieldDataDetail from './components/detail-component.vue';
...
const shallowRefs = {
  CommonFieldDataEdit: shallowRef(CommonFieldDataEdit),
...
useSetShortcutButtons('SystemCommonFieldDataList', extraButtons);
...
const toolbarClick = ({ code }) => {
  ...
  switch (code) {
    ...
    case 'new':
      ...
      popupConfig.componentProps.is = shallowRefs.CommonFieldDataEdit;
  ...
  if (operation?.click) {
    ...
    switch (name) {
      case 'save':
        break;
      case 'edit':
        popupConfig.containerProps.title = '编辑通用字段数据';
      ...
      case 'detail':
        if (operation?.containerType) {
          ...
          popupConfig.containerProps.showCancelBtn = true;
          popupConfig.componentProps.is = shallowRefs.CommonFieldDataDetail;
          popupConfig.componentProps.entityId = row.id;
