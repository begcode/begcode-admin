<template>
  <div data-cy="CommonFieldDataRelationHeading">
    <a-card
...
import CommonFieldDataDetail from './detail-component.vue';
import CommonFieldDataEdit from './form-component.vue';
import CommonFieldDataList from '../common-field-data-list.vue';
...
const config = {
  ...
  baseGridOptions: (): VxeGridProps => {
    return {
      ...
      showOverflow: true,
      keepSource: true,
      id: 'vxe_grid_commonFieldData_relation',
      printConfig: {
        columns: [
          // { field: 'name' },
        ],
...
const toolbarClick = ({ code }) => {
  ...
  switch (code) {
    ...
    case 'add': {
      ...
      popupConfig.componentProps.is = shallowRef(CommonFieldDataList);
    ...
    case 'new':
      ...
      popupConfig.containerProps.showOkBtn = true;
      popupConfig.containerProps.showCancelBtn = true;
      popupConfig.componentProps.is = shallowRef(CommonFieldDataEdit);
      popupConfig.componentProps.entityId = '';
      popupConfig.componentProps.baseData = props.baseData;
...
const rowClick = ({ name, data }) => {
  ...
  if (operation?.click) {
    ...
    switch (name) {
      case 'detail':
        popupConfig.componentProps.is = shallowRef(CommonFieldDataDetail);
