<template>
  <div data-cy="FillRuleItemRelationHeading">
    <a-card
...
import FillRuleItemDetail from './detail-component.vue';
import FillRuleItemEdit from './form-component.vue';
import FillRuleItemList from '../fill-rule-item-list.vue';
...
const config = {
  ...
  baseGridOptions: (): VxeGridProps => {
    return {
      ...
      showOverflow: true,
      keepSource: true,
      id: 'vxe_grid_fillRuleItem_relation',
      printConfig: {
        columns: [
          // { field: 'name' },
        ],
...
const toolbarClick = ({ code }) => {
  ...
  switch (code) {
    ...
    case 'add': {
      ...
      popupConfig.componentProps.is = shallowRef(FillRuleItemList);
    ...
    case 'new':
      ...
      popupConfig.containerProps.showOkBtn = true;
      popupConfig.containerProps.showCancelBtn = true;
      popupConfig.componentProps.is = shallowRef(FillRuleItemEdit);
      popupConfig.componentProps.entityId = '';
      popupConfig.componentProps.baseData = props.baseData;
...
const rowClick = ({ name, data }) => {
  ...
  if (operation?.click) {
    ...
    switch (name) {
      case 'detail':
        popupConfig.componentProps.is = shallowRef(FillRuleItemDetail);
