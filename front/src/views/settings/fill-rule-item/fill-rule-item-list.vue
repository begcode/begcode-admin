<template>
  ...
  <div data-cy="FillRuleItemHeading">
    <a-card
...
import FillRuleItemDetail from './components/detail-component.vue';
...
const shallowRefs = {
  FillRuleItemEdit: shallowRef(FillRuleItemForm),
  FillRuleItemDetail: shallowRef(FillRuleItemDetail),
};
...
const apis = {
  ...
  import: apiService.settings.fillRuleItemService.importExcel,
  export: apiService.settings.fillRuleItemService.exportExcel,
...
useSetShortcutButtons('SystemFillRuleItemList', extraButtons);
...
function toolbarClick({ code }) {
  ...
  switch (code) {
      ...
      if (records?.length > 0) {
        ...
        Modal.confirm({
          ...
          okButtonProps: {
            ...
            'data-cy-heading': 'fillRuleItemDeleteDialogHeading',
          } as any,
        });
      }
      break;
    ...
    case 'new':
      ...
      popupConfig.componentProps.is = shallowRefs.FillRuleItemEdit;
...
function rowClick({ name, data, params }) {
  ...
  if (operation?.click) {
    ...
    switch (name) {
      ...
      case 'edit':
        popupConfig.needSubmit = true;
        popupConfig.containerProps.title = '编辑填充规则条目';
      ...
      case 'detail':
        ...
        popupConfig.containerProps.showCancelBtn = true;
        popupConfig.componentProps.is = shallowRefs.FillRuleItemDetail;
        popupConfig.componentProps.entityId = row.id;
