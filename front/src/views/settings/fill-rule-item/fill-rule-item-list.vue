  ...
  switch (code) {
    ...
    case 'new':
      const containerConfig: any = {
        componentName: shallowRef(FillRuleItemEdit),
        entityId: '',
        title: '新建',
        okText: '保存',
        cancelText: '取消',
  ...
  if (operation?.click) {
    ...
    switch (name) {
      case 'save':
        break;
      case 'edit':
        containerConfig.componentName = shallowRef(FillRuleItemEdit);
        containerConfig.title = '编辑填充规则条目';
      ...
      case 'detail':
        if (operation?.containerType) {
          containerConfig.componentName = shallowRef(FillRuleItemDetail);
