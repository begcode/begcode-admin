<template>
  ...
  <div data-cy="SysFillRuleHeading">
    <a-card
    ...
    <a-card :bordered="false" class="bc-list-result-card" :bodyStyle="{ 'padding-top': '1px' }">
      <vxe-grid ref="xGrid" v-bind="gridOptions" v-on="gridEvents" data-cy="entityTable">
        ...
        <template #ruleItems_default="{ row, column }">
          <AvatarGroupInfo
            :value="row.ruleItems"
            :disabled="true"
            avatar-slot-name="default"
            avatar-slot-field="datePattern"
            avatar-tip-field="datePattern"
...
import SysFillRuleDetail from './components/detail-component.vue';
...
const shallowRefs = {
  SysFillRuleEdit: shallowRef(SysFillRuleForm),
  SysFillRuleDetail: shallowRef(SysFillRuleDetail),
  FillRuleItemList: shallowRef(FillRuleItemList),
...
useSetShortcutButtons('SystemSysFillRuleList', extraButtons);
...
function toolbarClick({ code }) {
  ...
  switch (code) {
      ...
      if (records?.length > 0) {
        ...
        Modal.confirm({
          ...
          okButtonProps: {
            ...
            'data-cy-heading': 'sysFillRuleDeleteDialogHeading',
          } as any,
        });
      }
      break;
    ...
    case 'new':
      ...
      popupConfig.componentProps.is = shallowRefs.SysFillRuleEdit;
...
function rowClick({ name, data, params }) {
  ...
  if (operation?.click) {
    ...
    switch (name) {
      ...
      case 'edit':
        popupConfig.needSubmit = true;
        popupConfig.containerProps.title = '编辑填充规则';
      ...
      case 'detail':
        ...
        popupConfig.containerProps.showCancelBtn = true;
        popupConfig.componentProps.is = shallowRefs.SysFillRuleDetail;
        popupConfig.componentProps.entityId = row.id;
      ...
      case 'delete':
        ...
        break;
      case 'ruleItemsColumnView': {
        popupConfig.containerProps.title = '关联填充规则条目';
        popupConfig.containerProps.okText = '';
        popupConfig.containerProps.cancelText = '关闭';
        popupConfig.needSubmit = false;
        popupConfig.containerProps.showOkBtn = false;
        popupConfig.containerProps.showCancelBtn = true;
        popupConfig.componentProps.is = shallowRefs.FillRuleItemList;
        const rowParams = {
          ...params,
          containerType: 'modal',
          cardExtra: [],
          ...
          gridOptions: {
            ...
            height: row.ruleItems?.length > 0 ? 50 * ((row.ruleItems || []).length + 3) : undefined,
          },
          source: 'SysFillRule',
          gridCustomConfig: {
            rowOperations: ['detail'],
            hideColumns: ['fillRule'],
          },
          field: 'ruleItems',
          query: {
            'id.in': row.ruleItems.map(item => item.id),
          },
        };
