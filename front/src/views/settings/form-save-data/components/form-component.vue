<template>
  <div :class="containerType === 'page' ? ['pb-44px'] : []" data-cy="FormSaveDataCreateUpdateHeading">
    <basic-form ref="formRef" v-bind="formProps">
      <template #resetBefore>
        <teleport to='[data-teleport="settings/form-save-data-edit-append-button"]' defer>
          <a-space>
...
import { FormSaveData, IFormSaveData } from '@/models/settings/form-save-data.model';
...
const formSaveDataId = ref<any>(null);
const formSaveData = reactive<IFormSaveData>(new FormSaveData());
const getEntityData = async (entityId: string | number) => {
  if (entityId) {
    formSaveDataId.value = entityId;
    const data = await apiService.settings.formSaveDataService.find(Number(entityId)).catch(() => null);
    if (data) {
      Object.assign(formSaveData, data);
    }
  } else {
    Object.assign(formSaveData, props.baseData);
  }
};
...
const saveOrUpdateApi = props.entityId ? apiService.settings.formSaveDataService.update : apiService.settings.formSaveDataService.create;
const submit = async (config = { submitToServer: true }) => {
  ...
  if (result.success) {
    Object.assign(formSaveData, result.data);
    if (!config.submitToServer) {
      return formSaveData;
    }
    const saveData = await saveOrUpdateApi(formSaveData).catch(() => false);
    if (saveData) {
      Object.assign(formSaveData, saveData);
