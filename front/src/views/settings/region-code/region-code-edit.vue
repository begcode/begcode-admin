<template>
  <PageWrapper v-bind="pageProps">
    <template #default>
      <RegionCodeForm ref="regionCodeFormRef" v-bind="formProps" @updateSaveButton="updateSaveButton"></RegionCodeForm>
    </template>
...
const regionCodeFormRef = ref<any>(null);
const pageProps = reactive({
  title: '编辑',
});
let regionCodeId = ref(route.params?.entityId as string);

const saveOrUpdate = async () => {
  const result = await regionCodeFormRef.value.submit();
  if (result) {
    if (!regionCodeId.value) {
      const { fullPath } = route;
      tabStore.closeTabByKey(fullPath, router).then(() => {
        go('/settings/region-code/' + result.id + '/edit', true);
      });
    }
  }
};
...
const operationsConfig = ref<any>([
  ...
  {
    title: regionCodeId.value ? '更新' : '保存',
    icon: '',
    type: 'primary',
    name: 'save',
    click: saveOrUpdate,
