...
export function useMenuSetting() {
  ...
  const getCalcContentWidth = computed(() => {
    let width = 0;
    if (unref(getIsTopMenu) || !unref(getShowMenu) || (unref(getSplit) && unref(getMenuHidden))) {
      width = 0;
      ...
      if (unref(getMixSideFixed) && unref(mixSideHasChildren)) {
        width = width + unref(getRealWidth);
      }
    } else {
      width = unref(getRealWidth);
    }
