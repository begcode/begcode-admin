...
Cypress.Commands.add('clickOnPasswordItem', () => {
  cy.get(userDropdownSelector).click();
  return cy.get(changePasswordMenuSelector).click();
});

Cypress.Commands.add('clickOnShowMenu', () => {
  cy.get(userDropdownSelector).click();
  return cy.get(changePasswordMenuSelector).click();
});
...
declare global {
  namespace Cypress {
    interface Chainable {
      ...
      clickOnShowMenu(): Cypress.Chainable;
